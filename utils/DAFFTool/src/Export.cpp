 ...
 *
 *  File:		Export.cpp
 *  Purpose:	DAFFTool export routines
 *  Version:    $Id: Export.cpp,v 1.3 2010/03/11 13:08:54 fwefers Exp $
...
int exportIR(DAFFContentIR* pContentIR, int iRecordIndex, const std::string& sFilename, int iView, bool bQuiet, bool bVerbose) {
	...
	for (int iFrame=0; iFrame<iFilterLength; iFrame++)
		for (int iChannel=0; iChannel<iChannels; iChannel++)
			pfDataInterleaved[iCursor++] = pfData[iChannel*iFilterLength + iFrame + iChannel];
	...
	if (!bQuiet) {
		printf("Exported impulse response to \"%s\" (%i samples | %i channels | %s Hz | %s)\n", 
			   ...
			   iFilterLength,
			   iChannels,
			   DAFFUtils::Float2StrNice((float) (pContentIR->getSamplerate()*0.001),1,false).c_str(),
