...
: m_pParentNode(NULL)
{
	m_pNodeAssembly = vtkAssembly::New();
}

SGNode::SGNode(FXVTK2::SGNode* pParentNode)
: m_pParentNode(NULL)
{
	m_pNodeAssembly = vtkAssembly::New();
...
SGNode::~SGNode() {
	...
	m_pNodeAssembly->Delete();
	FXVTK2_UNLOCK_VTK;
}

FXVTK2::SGNode* SGNode::GetRootNode() const {
	// Recursively traverse to root node
	return (m_pParentNode ? m_pParentNode->GetRootNode() : NULL);
}

FXVTK2::SGNode* SGNode::GetParentNode() const {
...
const FXVTK2::SGNode* SGNode::GetChildNode(int iIndex) const {
...
FXVTK2::SGNode* SGNode::GetChildNode(int iIndex) {
	assert( (iIndex > 0) && (iIndex < (int) m_vpChildNodes.size()) );
...
void SGNode::GetChildNodes(std::vector<const FXVTK2::SGNode*>& vpChildren) const {
...
void SGNode::GetChildNodes(std::vector<FXVTK2::SGNode*>& vpChildren) {
	if (m_vpChildNodes.empty()) {
		vpChildren.clear();
		return;
	}
...
bool SGNode::AddChildNode(FXVTK2::SGNode* pChild) {
	/*
...
bool SGNode::AddChildNodes(const std::vector<FXVTK2::SGNode*>& vpChildren) {
	bool bResult = true;
	for (std::vector<FXVTK2::SGNode*>::const_iterator cit=vpChildren.begin(); cit!=vpChildren.end(); ++cit)
		bResult &= AddChildNode(*cit);
	return bResult;
}

bool SGNode::RemoveChildNode(FXVTK2::SGNode* pChild) {
	for (std::vector<FXVTK2::SGNode*>::iterator it=m_vpChildNodes.begin(); it!=m_vpChildNodes.end(); ++it)
		if ((*it) == pChild) {
			// Assemblies lösen
			FXVTK2_LOCK_VTK;
...
bool SGNode::RemoveChildNodes(const std::vector<FXVTK2::SGNode*>& vpChildren) {
	bool bResult = true;
	for (std::vector<FXVTK2::SGNode*>::const_iterator cit=vpChildren.begin(); cit!=vpChildren.end(); ++cit)
	                          ...
	                          double ux, double uy, double uz)
{
	...
	vtkMatrix4x4* mViewUp = vtkMatrix4x4::New();
	mViewUp->Identity();
...
void SGNode::AddActor(vtkActor* pActor) {
	assert ( pActor != NULL );
...
void SGNode::RemoveActor(vtkActor* pActor) {
	assert ( pActor != NULL );
...
void SGNode::AddAssembly(vtkAssembly* pAssembly) {
...
void SGNode::RemoveAssembly(vtkAssembly* pAssembly) {
	assert ( pAssembly != NULL );
...
vtkAssembly* SGNode::GetNodeAssembly() {
